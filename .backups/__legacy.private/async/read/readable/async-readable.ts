...
import {
  AsyncFlowFactoryCollection,
  type AsyncFlowFactoryCollectionItemsToAsyncFlowCollectionItems,
} from '../../collection/factory/async-flow-factory-collection.js';
import { AsyncFlowCollection } from '../../collection/flow/async-flow-collection.js';
import {
  AsyncFlowFactory,
...
import { AsyncReadableInspectOptions } from './types/methods/async-readable-inspect-options.js';
...
export class AsyncReadable<GValue> extends AsyncFlowFactory<AsyncReader<GValue>> {
  ...
  override inspect({ next, ...options }: AsyncReadableInspectOptions<GValue> = {}): this {
  ...
  //                    error,
  //                    abort,
  //                  }: AsyncReadableInspectOptions<GValue> = {}): this {
  //   return new AsyncReadable<GValue>(async (signal?: AbortSignal): Promise<AsyncReader<GValue>> => {
  ...
  keepalive(): AsyncReadable<GValue> {
    return new AsyncReadable<GValue>(async (signal?: AbortSignal): Promise<AsyncReader<GValue>> => {
      let reader: AsyncReader<GValue> = await this.open(signal);
