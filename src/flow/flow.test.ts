  ...
  describe('static-methods', () => {
    describe('fromPushSource', () => {
      describe('dataRetentionTime=0', () => {
        ...
        beforeEach(() => {
          reader = Flow.fromPushSource<number>((_bridge: PushBridge<number>): void => {
            bridge = _bridge;
      ...
      it('should support async init and disposal', async () => {
        ...
        const reader: FlowReader<number> = Flow.fromPushSource<number>(
    ...
    describe('when', () => {
      it('should receive event on the edge', async () => {
        ...
        const reader = Flow.when(emitter, 'event').open(controller.signal);
    ...
    describe('combine', () => {
      ...
      it('should combine record flows', async () => {
        expect(
          await Flow.combine({
            a: Flow.fromArray([0, 1, 2]),
            b: Flow.fromArray([3, 4]),
          }).toArray(controller.signal),
        ).toEqual([
          { a: 0, b: 3 },
          { a: 1, b: 4 },
      ...
      it('should rejects with the corresponding error aggregation then disposed with error', async () => {
        const flow = Flow.combine([
          new Flow<number, []>(async function* () {
            try {
              yield 0;
            } finally {
              throw 'error0';
    ...
    describe('of', () => {
      it('should return expected values', async () => {
        expect(await Flow.of(0, 1, 2).toArray(controller.signal)).toEqual([0, 1, 2]);
      });
    });
    ...
    describe('fromPromiseFactory', () => {
      ...
      it('should support rejected promise', async () => {
        ...
        await expect(
          Flow.fromPromiseFactory(() => Promise.reject(error)).toArray(controller.signal),
    ...
    describe('fromIterable', () => {
      ...
      it('should support throw', async () => {
        ...
        const reader = Flow.fromIterable<void>(
    ...
    describe('fromAsyncIterable', () => {
      ...
      it('should support throw', async () => {
        ...
        const reader = Flow.fromAsyncIterable<void>(
  ...
  describe('methods', () => {
      ...
      it('should map the arguments', async () => {
        await expect(
          new Flow(async function* (
            ...
            .mapArguments((input: string): [number] => {
              return [Number(input)];
            })
            .toArray(controller.signal, '10'),
        ).resolves.toEqual([10]);
      });

      it('should map ONE arguments', async () => {
        await expect(
          new Flow(async function* (
            _ctx: FlowContext,
            input: number,
            ...
            .mapArguments(mapOneArgument<string, number>(Number))
            .toArray(controller.signal, '10'),
        ).resolves.toEqual([10]);
      });

      it('should set predefined arguments', async () => {
        await expect(
          new Flow(async function* (
            _ctx: FlowContext,
            input: number,
    ...
    describe('takeUntil', () => {
      it('should take values until sub flow completes', async () => {
        ...
        const reader: FlowReader<number> = Flow.fromArray<number>([0, 1, 2])
          .takeUntil(
            Flow.fromPushSource<void>((_bridge: PushBridge<void>): void => {
              bridge = _bridge;
            }).mapArguments(
              setArguments({
                dataRetentionTime: 100,
    ...
    describe('flatMap', () => {
      it('should chain Flows', async () => {
        expect(
          await Flow.fromIterable([0, 1, 2])
            .flatMap((value: number): Flow<number> => {
              return Flow.fromIterable([value * 2, value * 3]);
            })
            .toArray(controller.signal),
        ).toEqual([0, 0, 2, 3, 4, 6]);
      });
    ...
    describe('fork', () => {
      describe('maintainAlive=0', () => {
        it('should share emitted values', async () => {
          ...
          const flow: Flow<number> = Flow.fromPushSource<number>(
    ...
    describe('catch', () => {
      it('should be called when the flow errors', async () => {
        const spy = vi.fn();
        expect(
          await Flow.thrown(() => 'error')
            .catch((error: unknown) => {
              spy(error);
              return Flow.fromIterable([0, 1, 2]);
            })
            .toArray(controller.signal),
        ).toEqual([0, 1, 2]);
    ...
    describe('forEach', () => {
      it('iterates over all the values', async () => {
        ...
        await expect(
          Flow.fromIterable([0, 1, 2]).forEach(controller.signal, spy),
    ...
    describe('reduce', () => {
      it('performs the sum of data', async () => {
        expect(
          await Flow.fromIterable([0, 1, 2]).reduce<number>(
            controller.signal,
            (sum, v) => sum + v,
            0,
          ),
    ...
    describe('find', () => {
      it('should return a value if the value exists', async () => {
        expect(
          await Flow.fromIterable([0, 1, 2]).find<number>(
            controller.signal,
    ...
    describe('toReadableStream', () => {
      it('should emit expected values', async () => {
        const stream = Flow.fromArray([0, 1, 2]).toReadableStream();
      ...
      it('should support errors', async () => {
        const stream = Flow.thrown<number>(() => 'error').toReadableStream();
